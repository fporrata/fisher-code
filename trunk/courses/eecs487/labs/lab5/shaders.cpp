...
bool InitShaders(const char* vpname, const char *fpname, GLhandleARB* hpo) {
  if(!hpo)
    return false;

  bool okay = true;
  if(GLEW_ARB_shader_objects && GLEW_ARB_vertex_shader && GLEW_ARB_fragment_shader) {
    cerr << "init " << vpname << (fpname ? string(" and ")+fpname : string("")) << endl;
    ...
    string src;
    string vsname = vpname + string(".vs");
    ...
    string fsname = fpname + string(".fs");
    ...
    if(!linked) {
      cerr << "could not link shaders" << endl;
    }
