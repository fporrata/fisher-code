function [polyValue, ra] = Zern(r, theta, n, m)
...
if mod(n-mm, 2) == 0
    for k = 0:((n-mm)/2)
        ra = ra + (-1)^k*factorial(n-k) * r'.^(n-2*k) / (factorial(k) * ...
            factorial(0.5 * (n+mm) - k)*factorial(0.5*(n-mm)-k));
    end
    if m < 0
...
%                       r' .^ (n-2) / factorial(2 * n - 2);
                  for k = 0:1
                      ra(:, n+1, m+1) = ra(:, n+1, m+1)+(-1)^k*factorial(n-k) * r' .^(n-2*k) / ...
            (factorial(k)*factorial(0.5*(m+n)-k)*factorial(1-k));
                  end
              else%
