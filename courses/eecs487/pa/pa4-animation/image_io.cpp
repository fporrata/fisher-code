...
Image* ImageIO::LoadJPEG(const char* filename) {
...
Image* ImageIO::LoadPNG(const char* filename) {
  png_structp png_ptr;
  png_infop info_ptr;
  ...
  cerr << "png read: " << info_ptr->width << "x" << info_ptr->height << endl;
  cerr << "channels: " << (unsigned int)info_ptr->channels 
    << ", bit_depth = " << (unsigned int)info_ptr->bit_depth << endl;
  ...
  Image* image = new Image(info_ptr->width, info_ptr->height, 
                           info_ptr->channels);
  
  for(int row=0; row<(int)info_ptr->height; ++row) {
    memcpy(image->mutable_row(info_ptr->height - 1 - row), row_pointers[row], 
...
ImageIO::ImageFormatEnum ImageIO::recognize_image_format(const char* filename)
{
